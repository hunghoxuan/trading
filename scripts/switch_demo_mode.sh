...
#
# Usage:
#   bash scripts/switch_demo_mode.sh ea
#   bash scripts/switch_demo_mode.sh v2
#   bash scripts/switch_demo_mode.sh ctrader

MODE="${1:-}"
if [[ -z "${MODE}" ]]; then
  echo "Usage: bash scripts/switch_demo_mode.sh <ea|v2|ctrader>" >&2
  exit 1
fi
...
WEBHOOK_ENV="/opt/trading/webhook/.env"
V2_ENV="/opt/trading/scripts/v2_broker_executor.env"
