 ...
 * 
 * Usage:
 *   node scripts/utils/ai.js --list
 *   node scripts/utils/ai.js --model deepseek-coder "Analyze signal..."
 *   node scripts/utils/ai.js --provider ollama "What is the trend?"
 * 
...
async function main() {
  ...
  const globalConfigPath = path.join(os.homedir(), ".gemini/antigravity/provider_config.yaml");
  ...
  if (!prompt) {
    console.log("Usage: node scripts/utils/ai.js [--model <name> | --provider <name>] [--project] [--debug] \"prompt\"");
    process.exit(0);
  }
