...
## Current src heads
- `src/Hung - MSS.pine` -> `@file-version: 0327-15`
- `src/Hung - SMC.pine` -> `@file-version: 0327-15`
- `src/Hung - Core.pine` -> `@file-version: 0327-15`
...
- Result:
  ...
  - Follow-up cleanup completed in `0327-11`: Core global compat controls were removed and moved to model-driven config.
...
## One-pass 0327-9 (Required-Events Preset Normalization)
- Status: DONE
- Scope:
  - Added local preset resolver `get_data_required_events_preset(...)` in Core/SMC/MSS.
  - Replaced inline hardcoded `required_previous_events` strings in `process_data_init_strategy_defs()` with preset calls.
  - Preset mapping now centralized per file:
    - Core: `FIB -> BOS`, `SWEEP -> SWEEP`, others empty.
    - SMC: `BREAK_OUT -> MSS,BOS`, `REJECTION -> SWEEP`, others empty.
    - MSS: `SWEEP_MSS -> SWEEP,MSS`, `SWEEP_MSS_FVG -> SWEEP,MSS,BOS`, others empty.
- Result:
  - Required-event defaults are now managed at one point per indicator, reducing inline config drift and easing future model additions.

## One-pass 0327-10 (Trade Config Group Consolidation)
- Status: DONE
- Scope:
  - Core/SMC:
    - Reclassified trade controls group label from legacy to active:
      - `GROUP_TRADE_CONFIG = "3. Trade Config"`.
  - MSS:
    - Removed now-unused `GROUP_TRADE_CONFIG` constant after previous legacy toggle cleanup.
- Result:
  - Settings structure is cleaner and no longer signals deprecated flow where active controls still exist.

## One-pass 0327-11 (Core: Remove Remaining Global Trade Controls)
- Status: DONE
- Scope:
  - Core removed global trade config inputs:
    - `entryMaxStart`
    - `entryDynamicTp`
    - `signalSlProfile`
    - `signalGapRiskPct`
  - Removed related legacy constants/helpers:
    - `SL_PROFILE_*`
    - `get_data_trade_gap_profile_floors(...)`
  - Entry execution now uses per-model dynamic config from `EntryModelDef`:
    - Max active cap from `max_active`.
    - Dynamic TP behavior from `dynamic_tp_sl`.
    - Risk gap percent from `risk_zone_pct`.
  - Updated entry creation flow:
    - `process_data_add_strategy_entry(...)` now receives `dynamicTpCfg` and `riskPctCfg`.
    - `process_data_triggers_queue(...)` enforces per-model max active before entry creation.
  - Removed global post-loop pending prune branch that depended on global max-active.
- Result:
  - Core trade path is now model-driven end-to-end without remaining global trade controls.

## One-pass 0327-13 (SMC Stabilization: Remove Strictness/Score Globals)
- Status: DONE
- Scope:
  - Removed remaining SMC global tuning knobs:
    - `Strictness`
    - `Score Profile`
  - Simplified SMC config builders to stable preset mode:
    - `get_ui_cfg(...)` no longer depends on strictness level.
    - `get_data_local_cfg(...)` no longer depends on strictness/profile ids.
    - `get_data_post_touch_keep_max()` no longer takes profile id; uses stable default.
  - Updated all SMC call sites to simplified signatures.
- Result:
  - SMC now avoids runtime/global profile drift and stays on one stable preset path aligned with model-driven flow.

## Cleanup Line Status
- Status: COMPLETE
- Completed scope:
  - Bias/trend dashboard consistency and semantics stabilization.
  - EntryModel-driven direction/risk/event gating migration.
  - Legacy global trade-control removal where applicable (Core complete; SMC strict/score globals removed; MSS legacy globals removed earlier).
  - Versioned checkpoints for each pass in `src-versions`.

## One-pass 0327-14 (Bar-Only Working Zone + Limit Unification)
- Status: DONE
- Scope:
  - Unified working-zone constants across Core/SMC/MSS:
    - `WORKING_MAX_BARS = 5000`
    - `SAFE_HISTORY_OFFSET = 500`
  - Removed indicator resource caps for lines/labels/boxes and kept bar-only cap via:
    - `max_bars_back = 5000` in all 3 indicators.
  - Added first-bar working-zone initialization (once at `barstate.isfirst`) in all 3 indicators:
    - `workingFirstBar = bar_index - WORKING_MAX_BARS` (clamped at 0)
    - draws vertical marker line at `workingFirstBar`.
  - Converted active lookback behavior to fixed-origin expanding working zone:
    - `effectiveCurrLookbackBars` now expands from first bar (does not slide away).
    - replaced key draw-window checks with working-zone helper where needed.
  - Removed input bounds (`minval`/`maxval`) in Core/SMC/MSS.
  - Removed remaining global active-entry limit in SMC and switched to per-model cap only.
  - Bar-only mode cleanup:
    - SMC pdarray caps set to effectively uncapped large values.
    - MSS removed SR side/source cap pruning calls.
    - MSS replaced `EQ_SAFE_HISTORY_OFFSET` usage with unified `SAFE_HISTORY_OFFSET`.
- Result:
  - Runtime limiting is now dominated by bar-window logic (`WORKING_MAX_BARS`) instead of mixed caps/bounds.

## One-pass 0327-15 (WorkingMaxBars Tune)
- Status: DONE
- Scope:
  - Updated unified working window constant across Core/SMC/MSS:
    - `WORKING_MAX_BARS: 5000 -> 672`
  - Kept `max_bars_back = 5000` unchanged as compile-time safety headroom.
- Result:
  - Active detection/drawing workload is reduced to 1-week-at-15m equivalent while preserving history-buffer safety.

## One-pass 0327-16 (WorkingMaxBars Enforcement Fix)
- Status: DONE
- Scope:
  - SMC/MSS working-zone is now evaluated against current rolling window (`bar_index - WORKING_MAX_BARS`) instead of stale first-bar anchor behavior.
  - Added per-bar working-line update to visualize real active zone boundary.
  - Added hard prune for out-of-window objects:
    - SMC zones (`chartCtx.zones`)
    - MSS levels (`chartCtx.levels`)
  - Added MSS seed guards to block SR/LIQ creation when source pivot is outside working zone.
  - Aligned effective lookback to current working window width.
- Result:
  - `WORKING_MAX_BARS` now materially constrains active pdarray/level objects on chart.
