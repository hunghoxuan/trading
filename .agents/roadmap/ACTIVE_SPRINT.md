...
4. EntryModel schema lift into type-level model defs (`0326-53`):
- Extended `EntryModelDef` with dynamic trade fields and moved defaults into model init entries.
- Replaced local switch-based config with schema lookup from `strategyDefs`.
- Migrated SMC/MSS queue entry mode behavior to per-model `entry_mode`.
5. Tick dashboard refresh pass (`0403-25`):
- Enabled SMC realtime intrabar execution (`calc_on_every_tick = true`) so dashboard updates do not wait for candle close.
- Kept Core trade->dashboard ordering explicit to ensure dashboard reflects latest visual/trade state each tick.
## Change Log
