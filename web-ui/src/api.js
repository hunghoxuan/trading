...
export const api = {
  ...
  storageStats: () => get("/system/storage/stats"),
  storageCleanup: (target, userId = "") => post("/system/storage/cleanup", { target, userId }),
  listCache: () => get("/system/cache"),
  deleteCache: (key = "", source = "") => del(`/system/cache?key=${encodeURIComponent(key)}&source=${encodeURIComponent(source)}`),
